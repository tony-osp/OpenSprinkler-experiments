		...
		case 0x02: // WRQ
			{
				...
				Serial.println(fname);
				m_curBlock = 0;
				if (!m_theFile.open(fname, O_WRITE | O_CREAT))
					SendERR(6, "File Open ERR");
				else
					SendACK();
				break;
			}
...
void tftp::SendERR(int code, const char * msg)
{
	...
	strncpy((char*)packetBuffer+4,msg, sizeof(packetBuffer) - 4);
	Serial.println(msg);
	m_udp.beginPacket(m_remoteIP, m_remotePort);
